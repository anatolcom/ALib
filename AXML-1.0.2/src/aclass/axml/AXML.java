...
//---------------------------------------------------------< XML entity >---
 private static final char ENTITY_AMP[] =
 {
  'a', 'm', 'p'
 };     //  &amp;  &
 private static final char ENTITY_LT[] =
 {
  'l', 't'
 };         //  &lt;   <
 private static final char ENTITY_GT[] =
 {
  'g', 't'
 };         //  &gt;   >
 private static final char ENTITY_APOS[] =
 {
  'a', 'p', 'o', 's'
 }; //  &apos; '
 private static final char ENTITY_QUOT[] =
 {
  'q', 'u', 'o', 't'
 }; //  &quot; "
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
 /**
  * Метод определяет является ли содержимое в тексте TEXT в позиции POS равным по значению переносу
  * строки.<br/>
...
//---------------------------------------------------------------------------
 ...
 private static boolean isID(final String TEXT, final int POS, final char ID[], ARefByte Len) throws AException
 {
  if (TEXT == null) throw new AException("TEXT=null");
  if (Len == null) throw new AException("Len=null");
  if (POS >= TEXT.length()) throw new AException("POS: " + POS + " >= SIZE");
...
//---------------------------------------------------------------------------
 /**
  * Метод выбирает слово из текста TEXT начиная с позиции POS и записывает его в переменную
  * Word.value.<br/>
  *
...
//---------------------------------------------------------------------------
 /**
  * Метод выбирает номер из текста TEXT начиная с позиции POS и записывает его в переменную
  * Number.value.<br/>
  *
  * @param TEXT - Текст для синтаксического анализа.
  * @param POS - Позиция сравнения.
 ...
 private static int getNumber(final String TEXT, final int POS, ARefInt number) throws AException
 {
  ...
  if (POS >= len) throw new AException("POS: " + POS + " >= SIZE");
  ...
  try
  {
   number.value = Integer.parseInt(str);
  }
  catch (NumberFormatException ex)
  {
   throw new AException("str convert to int ERROR", ex);
  }
  return pos;
 }
//---------------------------------------------------------------------------
 /**
  * Метод выбирает тег из текста TEXT начиная с позиции POS и записывает его в переменную
  * Tag.value.<br/>
  *
  * @param TEXT - Текст для синтаксического анализа.
  * @param POS - Позиция сравнения.
...
//---------------------------------------------------------------------------
 /**
  * Метод выбирает параметр тега из текста TEXT начиная с позиции POS и записывает его в переменную
  * Param.value.<br/>
  *
  * @param TEXT - Текст для синтаксического анализа.
  * @param POS - Позиция сравнения.
...
//---------------------------------------------------------------------------
 /**
  * Метод выбирает спецсимвол тега из текста TEXT начиная с позиции POS и записывает его в
  * переменную Param.value.<br/>
  *
  * @param TEXT - Текст для синтаксического анализа.
  * @param POS - Позиция сравнения.
...
//---------------------------------------------------------------------------
 /**
  * Метод выберает из тега Tag параметры и записывает их в список параметров ParamList.<br/>
  *
  * @param Tag - Ссылка на тег.
...
//---------------------------------------------------------------------------
 ...
 private static String EntityToSpecSymbol(final char ENTITY[])
 {
  ...
  if (test.contains("null")) sendGlobalError("EntityToSpecSymbol", "contains null");
  return test;
 }
//---------------------------------------------------------------------------
 /**
...
//---------------------------------------------------------------------------
 /**
  * Метод преобразует данные Node в XML код, и в соответсвии с уровнем LEVEL отодвигает код от края
  * документа на расстояние SPACE.<br/>
  *
 ...
 public String NodeToXML(AXMLNode Node, final int LEVEL, final int SPACE) throws AException
 {
  if (Node == null)
  {
   sendError("NodeToXML", "NodeNode=null");
   return null;
  }
  String str = "";
...
//---------------------------------------------------------------------------
 ...
 public static String StrToXMLStr(String STR, boolean FORMATED) throws AException
 {
  ...
  while (true)
  {
   ...
   if (CHAR == 0) sendGlobalError("StrToXMLStr", "contains null");
//   if(CHAR<=32)
//   {
//    if(!FORMATED)
//    {
...
//   }
   if (CHAR < 32)
   {
    if (FORMATED) xmlstr.append(CharToSpecSymbol(CHAR));
    pos++;
    continue;
   }
   if (CHAR == 32)
   {
    ...
    else xmlstr.append(CharToSpecSymbol(CHAR));
    pos++;
    continue;
   }
//   first=true;//??????
   if (CHAR > 127)
   {
    xmlstr.append(CharToSpecSymbol(CHAR));
    pos++;
    continue;
   }
   first = true;
   ...
   if (isID(TEXT, pos, ID_TAG_BEGIN, Len))
   {
    ...
    switch (type)
    {
     case TT_ERROR://--------------------------------<   Incorrect tag   >---
      sendError("fromXML", "getTagType return ERROR");
      return false;

     case TT_INSTRUCTION://-------------<   <?ProcessingInstructions?>   >---
      sendInfo("fromXML", "Parse for <? ... ?> don't realised");
      break;
...
//---------------------------------------------------------------------------
 public static String trimXML(String xml) throws AException
 {
  xml = xml.trim();
