...
//---------------------------------------------------------------------------
 public static ArrayList<AZipItem> readZipApache(InputStream in) throws AException
 {
  try
  {
   ...
   catch(Exception ex){sendGlobalError("readZipApache","Read from ZipInputStream Exception: "+ex.getMessage(),null);}
   return includeList;
  }
  catch(Exception ex){throw new AException("Read from zip error",ex);}
 }
...
//	zop.setEncoding("UTF-8");
   ...
   catch(Exception ex){sendGlobalError("writeZipApache","Write to ZipOutputStream Exception: "+ex.getMessage(),null);}
  }
  catch(Exception ex){throw new AException("Write to zip error",ex);}
 }
//---------------------------------------------------------------------------
