...
/**
  ...
  */
 protected void PSetCount(final int value)
 {
  if(value<0)
  {
   sendError("PSetCount","Count<0",value);
   return;
  }
 ...
 */
 protected int PAdd(Object value)
 {
  if(fcount==MaxInt)
  {
   sendError("PAdd","Count=Max",fcount);
   return fcount-1;
  }
  APtrElement enew=new APtrElement();
  CInsert(enew,null,fcount);
 ...
 * @return Позиция элемента в массиве.
 */
 protected int PInsert(Object value,final int index)
 {
  if(fcount==MaxInt)
  {
   sendError("PInsert","Count=Max",fcount);
   return fcount-1;
  }
  if(index<0)
  {
   sendError("PInsert","index<0",index);
   return index;
  }
  if(index>fcount)
  {
   sendError("PInsert","index>Count",index);
   return index;
  }
  APtrElement enew=new APtrElement();
 ...
 */
 protected int PNew()
 {
  if(fcount==MaxInt)
  {
   sendError("PNew","Count=Max",fcount);
   return fcount-1;
  }
  APtrElement enew=new APtrElement();
  CInsert(enew,null,fcount);
 ...
 * @return Позиция элемента в массиве.
 */
 protected int PNew(final int index)
 {
  if(fcount==MaxInt)
  {
   sendError("PNew","Count=Max",fcount);
  ...
  if(index<0)
  {
   sendError("PNew","index<0",index);
   return index;
  }
  if(index>fcount)
  {
   sendError("PNew","index>Count",index);
   return index;
  }
  APtrElement enew=new APtrElement();
  if(index==fcount)CInsert(enew,null,fcount);
 ...
 */
 protected void PDelete(final int index)
 {
  if(index<0)
  {
   sendError("PDelete","index<0",index);
   return;
  }
  if(index>=fcount)
  {
   sendError("PDelete","index>=Count",index);
   return;
  }
 ...
 */
 protected void PSwap(final int a,final int b)
 {
  if(a==b)return;
  if(a<0)
  {
   sendError("PSwap","a<0",a);
   return;
  }
  if(b<0)
  {
   sendError("PSwap","b<0",b);
   return;
  }
  if(a>=fcount)
  {
   sendError("PSwap","a>=Count",a);
   return;
  }
  if(b>=fcount)
  {
   sendError("PSwap","b>=Count",b);
   return;
  }
 ...
 * @param b - Позиция элемента b.
 */
 protected void PMove(final int a,final int b)
 {
  if(a==b)return;
  if(a<0)
  {
   sendError("PMove","a<0",a);
   return;
  }
  if(b<0)
  {
   sendError("PMove","b<0",b);
   return;
  }
  if(a>=fcount)
  {
   sendError("PMove","a>=Count",a);
   return;
  }
  if(b>=fcount)
  {
   sendError("PMove","b>=Count",b);
   return;
  }
 ...
 * @param index - Позиция элемента.
 * @return Ссылка на элемент,<br/>
 *         null - если указанная позиция неверна.
 */
 protected Object PGetPtr(final int index)
 {
  if(index<0)
  {
   sendError("PGetPtr","index<0",index);
   return null;
  }
  if(index>=fcount)
  {
   sendError("PGetPtr","index>=Count",index);
   return null;
  }
  CfenGoTo(index);
  return fen.ptr;
 ...
 *         null - если указанная позиция неверна.
 */
 protected Object PGetPtrConst(final int index)//const
 {
  if(index<0)
  {
   sendError("PGetPtrConst","index<0",index);
   return null;
  }
  if(index>=fcount)
  {
   sendError("PGetPtrConst","index>=Count",index);
   return null;
  }
 ...
 */
 protected void PSetPtr(final int index,Object Ptr)
 {
  if(index<0)
  {
   sendError("PSetPtr","index<0",index);
   return;
  }
  if(index>=fcount)
  {
   sendError("PSetPtr","index>=Count",index);
   return;
  }
  CfenGoTo(index);
  fen.ptr=Ptr;
