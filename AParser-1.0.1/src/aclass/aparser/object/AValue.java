...
//---------------------------------------------------------------------------
 public final void set(byte[] value)throws AException                   //???
 {
  ...
  if(isDynamicSize())
  {
   ...
   if(value.length!=getConstSize()*cnt)throw new AException("value.length: " + value.length + " != ConstSize*cnt");
  }
  else 
  {
   if(value.length!=getConstSize())throw new AException("value.length: " + value.length + " != ConstSize");
  } 
