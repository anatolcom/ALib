...
//---------------------------------------------------------------------------
 ...
 {
  AOperation operation=obj.getOperationList().get("this");
  AUnion input=new AUnion(operation.getInputHeader());
  AUnion output=new AUnion(operation.getOutputHeader());
  operation.process(input,output);
  AValue vSizeof=(AValue)output.get("sizeof");
  Integer sizeof=vSizeof.getInt();
  if(sizeof==null)sendGlobalError("print","value sizeof not initialized",null); //throw new AException("value sizeof not initialized");
  ...
  if(obj.typeof().isType(AValue.UNION))
  {
   ...
   for(int q=0;q<union.count();q++)
   {
    ...
    str+=item.typeof().Name+" "+union.getHeader().get(q).getName()+"="+print(item);
   }
   return "("+str+")"+"$"+sizeof;
  }
