...
/**
 * //------------------
 * union U(int a=1,str b="param",char[] c=['A','B','C']); //описание со значениями по умолчанию
 * U u;//объявление
 * //------------------
 * union U(int a,str b,char[] c)u=new U(1,"param",['A','B','C']);//описание с объявлением с присвоением
 * u;//использование
 * //------------------
 * union U(int a,str b,char[] c);//описание
 * U u=new U(1,"param",['A','B','C']);//присвоение
 * //------------------
 * union U(int a,str b,char[] c); //описание
 * U u=new U;
 * u.a=1;                         //присвоение...
...
//---------------------------------------------------------------------------
 public AUnion(AUnionHeader header) throws AException
 {
  ...
  for(int q=0;q<header.count();q++)array[q]=header.get(q).getCopy();
 }
//---------------------------------------------------------------------------
 public AUnionHeader getHeader()throws AException{return new AUnionHeader(header);}
//---------------------------------------------------------------------------
...
//---------------------------------------------------------------------------
 public int getIndex(String name)throws AException{return header.getIndex(name);}
//---------------------------------------------------------------------------
 public int count(){return array.length;}
//---------------------------------------------------------------------------
 public boolean exists(String name){return header.exists(name);}
//---------------------------------------------------------------------------
 @Override
...
//---------------------------------------------------------------------------
 ...
 {
  sendError("assignData","under development",null);
