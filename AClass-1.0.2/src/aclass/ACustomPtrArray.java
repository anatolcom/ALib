 ...
 */
 protected int PInsert(Object value,final int index)throws AException
 {
  ...
  if(index>fcount)throw new AException("index>Count",index);
  APtrElement enew=new APtrElement();
 ...
 */
 ...
 {
  if(a==b)return;
  if(a<0)throw new AException("a<0",a);
  if(b<0)throw new AException("b<0",b);
  if(a>=fcount)throw new AException("a>=Count",a);
  if(b>=fcount)throw new AException("b>=Count",b);
