...
//   AClass.sendGlobalInfo("soapInvoke","ContentEncoding: "+connection.getContentEncoding(),null);
   ...
   try
   {
    ...
    if (in == null) AClass.sendGlobalError(ASOAP.class.getName(), "invoke", "connection.getInputStream() return null", null);
   }
   catch (IOException ex)
   {
    ...
    if (in == null) AClass.sendGlobalError(ASOAP.class.getName(), "invoke", "connection.getErrorStream() return null. " + ex.getMessage(), null);
   }
   if (in == null)
   {
    String respmsg = "";
   ...
   if (error)
   {
    AClass.sendGlobalWarning("invoke", "ResponseCode " + respcode, null);
...
//---------------------------------------------------------------------------
 ...
 public static String soapInvoke(String requestMessage, String endpoint, String soapActoin, String wsaAction) throws AException
 {
  ...
  return soapInvoke(requestMessage, endpoint, soapActoin, wsaAction, connectTimeout, readTimeout);
 }
//---------------------------------------------------------------------------
 /**
  * Выполняется SOAP запрос.<br/>
...
//---------------------------------------------------------------------------
// @Deprecated
// public static byte[] soapInvoke(byte[] request,String endpoint,String soapActoin)throws AException //!!!
// {
//  return soapInvoke(request,endpoint,soapActoin,null);
// }
//---------------------------------------------------------------------------
// @Deprecated
// public static String soapInvoke(String request,String endpoint,String soapActoin)throws AException
// {
//  System.out.println("soapInvoke is Deprecated");
//  return soapInvoke(request,endpoint,soapActoin,null);
// }
//---------------------------------------------------------------------------
 public static SOAPMessage soapMessageFromXml(String xml) throws AException
 {
  try
  {
   ...
   SOAPMessage response = messageFactory.createMessage(null, responseIn);
   responseIn.close();
   return response;
  }
  catch (IOException ex)
...
//---------------------------------------------------------------------------
 public static String generateWsSecurity(String username, String password) throws AException
 {
  try
  {
   ...
   password = HexBin.encode(md5.digest(password.getBytes())).toLowerCase();
   ...
   return "<wsse:Security xmlns:wsse=\"http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-wssecurity-secext-1.0.xsd\">"
           ...
           + "<wsse:Username>" + username + "</wsse:Username><wsse:Password Type=\"http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-username-token-profile-1.0#PasswordDigest\">" + b64e.encode(digest) + "</wsse:Password>"
           + "<wsse:Nonce EncodingType=\"http://docs.oasis-open.org/wss/2004/01/oasis-200401-wss-soap-message-security-1.0#Base64Binary\">" + b64e.encode(nonce.getBytes()) + "</wsse:Nonce>"
